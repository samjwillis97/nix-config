  ...
  async setup(context) {
    ...
    context.registerSecretResolver(async (secretName) => {
      ...
      if (config.secretMapping && config.secretMapping[secretName]) {
        ...
        if (config.debug) {
          console.error(
            `[RQP-Secrets] Fetching ${secretName} from ${secretPath} using ${config.provider}`
          );
        }
    ...
    if (config.secretMapping) {
      console.error(
        `[RQP-Secrets] Initialized with ${Object.keys(config.secretMapping).length} mapped secrets for ${config.provider}`
      );
    } else {
