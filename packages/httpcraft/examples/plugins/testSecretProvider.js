  ...
  async setup(context) {
    ...
    context.registerSecretResolver(async (secretName) => {
      ...
      if (config.secretMapping && config.secretMapping[secretName]) {
        ...
        if (config.debug) {
          console.error(
            `[TestSecrets] Providing secret ${secretName}: ${secretValue.substring(0, 4)}...`
          );
        }
    ...
    console.error(
      `[TestSecrets] Registered secret resolver with ${Object.keys(config.secretMapping || {}).length} secrets`
    );
  },
};
