...
describe('Chain Execution Integration', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(variableResolver.createContext).mockImplementation(
      (cliVars, profiles, apiVars, endpointVars) => {
        const context = {
          cli: cliVars || {},
          env: {},
          profiles: profiles || {},
  ...
  it('should support verbose output for chains', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '[CHAIN] Starting execution of chain: userWorkflow'
    );
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '[CHAIN] Description: Create a user and then retrieve it'
    );
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '[CHAIN] Step getCreatedUser completed successfully'
    );
...
`;
      ...
      expect(firstStep.request).toHaveProperty(
        'url',
      ...
      expect(secondStep.request).toHaveProperty(
        'url',
