  ...
  describe('Secret Variable Resolution', () => {
    it('should resolve secret variables from environment', async () => {
      const { stderr } = await execFileAsync(
        'node',
        [cliPath, 'httpbin', 'testSecrets', '--config', testConfigPath, '--dry-run'],
        {
          env: {
            ...process.env,
    ...
    it('should resolve secrets in request body', async () => {
      const { stderr } = await execFileAsync(
        'node',
        [cliPath, 'httpbin-body', 'testSecretInBody', '--config', testConfigPath, '--dry-run'],
        {
          env: {
            ...process.env,
    ...
    it('should throw error for undefined secret variables', async () => {
      try {
        const result = await execFileAsync(
          'node',
          [cliPath, 'httpbin', 'testUndefinedSecret', '--config', testConfigPath],
          {
            env: {
              ...process.env,
    ...
    it('should work with verbose output', async () => {
      ...
      const { stderr } = await execFileAsync(
        'node',
        [cliPath, 'httpbin', 'testSecrets', '--config', testConfigPath, '--dry-run', '--verbose'],
        {
          env: {
            ...process.env,
    ...
    it('should mask secrets in verbose mode during actual requests', async () => {
      const { stderr } = await execFileAsync(
        'node',
        [cliPath, 'httpbin', 'testSecrets', '--config', testConfigPath, '--verbose'],
        {
          env: {
            ...process.env,
