...
describe('Phase 9 Integration - Modular API Imports (T9.1)', () => {
  ...
  describe('Directory-based API Loading', () => {
    it('should execute requests using APIs loaded from directory', async () => {
      ...
      await fs.writeFile(
        path.join(apisDir, 'httpbin.yaml'),
        `
httpbin:
  baseUrl: "${mockBaseUrl}"
  endpoints:
...
`
      ...
      expect(response.headers['x-test-source'] || response.headers['X-Test-Source']).toBe(
        'modular-api'
      );
    });
...
`
  ...
  describe('Mixed Import Methods', () => {
    it('should handle both directory and individual file imports', async () => {
      ...
      await fs.writeFile(
        path.join(apisDir, 'dir-service.yaml'),
        `
dirService:
  baseUrl: "${mockBaseUrl}"
  endpoints:
...
`
      ...
      try {
        await execFile('node', [cliPath, 'someApi', 'someEndpoint', '--config', configFile]);
...
`
  ...
  describe('Tab Completion Integration', () => {
    it('should include modular APIs in completion', async () => {
      ...
      await fs.writeFile(
        path.join(apisDir, 'completion-test.yaml'),
        `
completionTest:
  baseUrl: "${mockBaseUrl}"
  endpoints:
