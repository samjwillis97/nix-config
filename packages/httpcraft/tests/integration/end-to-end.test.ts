...
`;
      ...
      const getResult = await runHttpCraft(['httpbin', 'get-json', '--config', configFile]);
      ...
      expect(
        headerResponseData.headers.authorization || headerResponseData.headers.Authorization
      ).toBe('Bearer secret-env-token-123');
...
`;
      ...
      expect(responseData.headers.authorization || responseData.headers.Authorization).toBe(
        'Bearer plugin-token-123'
      );
      expect(responseData.headers['x-timestamp'] || responseData.headers['X-Timestamp']).toMatch(
        /^\d+$/
      );

      // Verify pre-request hook worked
      expect(responseData.headers['x-plugin-added'] || responseData.headers['X-Plugin-Added']).toBe(
        'true'
      );
      expect(
        responseData.headers['x-plugin-config'] || responseData.headers['X-Plugin-Config']
      ).toBe('test-env');

      // Verify post-response hook worked
      expect(
        responseData.headers['x-processed-by-plugin'] ||
          responseData.headers['X-Processed-By-Plugin']
      ).toBe('true');
    });
...
`;
      ...
      expect(devResponseData.headers['x-api-key'] || devResponseData.headers['X-Api-Key']).toBe(
        'dev-api-key-12345'
      );
      expect(devResponseData.headers.authorization || devResponseData.headers.Authorization).toBe(
        'Token prod-api-key-12345'
      );
      ...
      expect(prodResponseData.headers['x-api-key'] || prodResponseData.headers['X-Api-Key']).toBe(
        'prod-api-key-12345'
      );
    });
  });
...
`;
      ...
      const createResponseBody =
        typeof createStep.response.body === 'string'
          ? JSON.parse(createStep.response.body)
          : createStep.response.body;
      const combineResponseBody =
        typeof combineStep.response.body === 'string'
...
`;
      ...
      const result = await runHttpCraft(['httpbin', 'test', '--config', configFile, '--dry-run']);
...
`
  ...
  describe('Modular Configuration Loading', () => {
    it('should load APIs and chains from directories', async () => {
      ...
      // Create API files
      await fs.writeFile(
        path.join(apisDir, 'service1.yaml'),
        `
service1:
  baseUrl: "${mockBaseUrl}"
  endpoints:
...
`
      ...
      await fs.writeFile(
        path.join(apisDir, 'service2.yaml'),
        `
service2:
  baseUrl: "${mockBaseUrl}"
  endpoints:
...
`
      ...
      // Create chain file
      await fs.writeFile(
        path.join(chainsDir, 'workflow.yaml'),
        `
test-workflow:
...
`;
      ...
      const apiResult = await runHttpCraft(['service1', 'test1', '--config', configFile]);
      ...
      const chainResult = await runHttpCraft(['chain', 'test-workflow', '--config', configFile]);
...
`;
      ...
      const result = await runHttpCraft(['httpbin', 'test-dynamic', '--config', configFile]);
      ...
      expect(requestData.guid1).toMatch(
        /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/
      );
      expect(requestData.guid2).toMatch(
        /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/
      );
