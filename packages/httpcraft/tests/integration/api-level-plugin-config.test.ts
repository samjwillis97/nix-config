...
`;
  ...
  it('should resolve variables in API-level plugin configurations from CLI variables', async () => {
    ...
    const pluginConfig = JSON.parse(
      response.headers['x-plugin-config'] || response.headers['X-Plugin-Config']
    );
  ...
  it('should resolve variables in API-level plugin configurations from profiles', async () => {
    const { stdout } = await execFileAsync(
      'node',
      [cliPath, 'test-api', 'test-endpoint', '--config', configPath, '--profile', 'test'],
      {
        env: { ...process.env, TEST_ENV_VAR: 'envTestValue' },
      }
    );
...
`;
    ...
    try {
      ...
      expect(error.stderr).toContain(
        'Failed to resolve variables in API-level plugin configuration'
      );
...
`;
    ...
    const result = await execFileAsync(
      'node',
      [cliPath, 'test-api', 'test-endpoint', '--config', configFile, '--profile', 'test'],
      {
        env: { ...process.env, TEST_ENV_VAR: 'envTestValue' },
      }
    );
    ...
    expect(responseData.headers['x-auth-url'] || responseData.headers['X-Auth-URL']).toBe(
      `${mockBaseUrl}/auth`
    );
    expect(responseData.headers['x-client-id'] || responseData.headers['X-Client-ID']).toBe(
      'test-client-id'
    );
  });
});
