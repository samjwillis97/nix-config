...
describe.skip('npm Plugin Loading Integration Tests (T10.7)', () => {
  ...
  it('should handle missing npm package with informative error', async () => {
    ...
    try {
      ...
      await expect(pluginManager.loadPlugins([pluginConfig], tempDir)).rejects.toThrow(
        "Failed to load plugin 'missingPlugin' from npm package 'httpcraft-missing-plugin': Cannot find module 'httpcraft-missing-plugin'"
      );
