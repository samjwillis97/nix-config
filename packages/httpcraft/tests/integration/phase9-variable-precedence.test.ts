...
describe('T9.7: Complete Variable Precedence Order', () => {
  ...
  describe('Full Precedence Chain Tests', () => {
    ...
    it('should handle mixed scoped and unscoped variables in same template', async () => {
      ...
      const template =
        'User: {{userId}}, Timeout: {{timeout}}, Version: {{api.version}}, Env: {{env.NODE_ENV}}, Secret: {{secret.API_KEY}}, Token: {{plugins.authPlugin.getToken}}, Limit: {{defaultLimit}}, Timestamp: {{$timestamp}}';
    ...
    it('should throw error when variable cannot be resolved at any level', async () => {
      ...
      await expect(resolver.resolve('{{nonExistentVar}}', context)).rejects.toThrow(
        "Variable 'nonExistentVar' could not be resolved"
      );
    });
  });
