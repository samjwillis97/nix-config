  ...
  describe('handleChainCommand', () => {
    ...
    it('should handle chain execution failure', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Chain execution failed: Step 'createUser' failed: HTTP 400: Bad Request"
      );
    });
    ...
    it('should exit with error when chain not found', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Error: Chain 'nonexistentChain' not found in configuration"
      );
    });
      ...
      it('should combine default profiles with CLI profiles', async () => {
        const config: HttpCraftConfig = {
          ...
          chains: {
            testChain: {
              vars: {},
              steps: [{ id: 'step1', call: 'testApi.test' }],
            },
          },
          apis: {
            testApi: {
    ...
    describe('verbose output for profile operations', () => {
      ...
      it('should show profile loading information in verbose mode', async () => {
        ...
        expect(stderrSpy).toHaveBeenCalledWith(
          '[VERBOSE]   Final profile order: base, env, user\n'
        );
      });
