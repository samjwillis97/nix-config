...
`;
    ...
    it('should throw error for invalid plugin path', async () => {
      ...
      await expect(pluginManager.loadPlugins([pluginConfig], process.cwd())).rejects.toThrow(
        "Failed to load plugin 'invalidPlugin'"
      );
    });
...
`;
      ...
      await expect(pluginManager.loadPlugins([pluginConfig], process.cwd())).rejects.toThrow(
        'does not export a valid Plugin object with a setup method'
      );
    });
  });
...
`;
      ...
      await expect(pluginManager.executePreRequestHooks(request)).rejects.toThrow(
        "Pre-request hook failed in plugin 'errorPlugin': Hook error"
      );
    });
  });
...
`;
      ...
      await expect(pluginManager.executePostResponseHooks(request, response)).rejects.toThrow(
        "Post-response hook failed in plugin 'errorPlugin': Post-response hook error"
      );
    });
...
export default {
  ...
  describe('T10.7: npm Plugin Loading', () => {
    ...
    it('should validate that either path or npmPackage is specified', async () => {
      ...
      await expect(pluginManager.loadPlugins([pluginConfig], process.cwd())).rejects.toThrow(
        "Plugin 'invalidPlugin' must specify either 'path' or 'npmPackage'"
      );
    });
    ...
    it('should validate that both path and npmPackage cannot be specified', async () => {
      ...
      await expect(pluginManager.loadPlugins([pluginConfig], process.cwd())).rejects.toThrow(
        "Plugin 'invalidPlugin' cannot specify both 'path' and 'npmPackage'"
      );
    });
    ...
    it('should handle npm package not found error', async () => {
      ...
      await expect(pluginManager.loadPlugins([pluginConfig], process.cwd())).rejects.toThrow(
        "Failed to load plugin 'missingPlugin' from npm package 'nonexistent-plugin'"
      );
    });
    ...
    it('should handle npm plugin without valid setup method', async () => {
      ...
      await expect(pluginManager.loadPlugins([pluginConfig], process.cwd())).rejects.toThrow(
        "Plugin from npm package 'invalid-npm-plugin' does not export a valid Plugin object with a setup method"
      );
    ...
    it('should preserve npm package source in error messages', async () => {
      ...
      await expect(pluginManager.loadPlugins([pluginConfig], process.cwd())).rejects.toThrow(
        "Failed to load plugin 'failingPlugin' from npm package 'failing-npm-plugin': Plugin setup failed"
      );
...
`;
      ...
      const apiPluginManager = await pluginManager.loadApiPlugins(
        apiConfigs,
        tempDir,
        variableContext
      );
      ...
      const apiConfigs: PluginConfiguration[] = [
        {
          ...
          config: {
            authorizationUrl:
              '{{plugins.configProvider.getDataSourceConfig("authorizationUrl", "{{dataSource}}")}}',
            tokenUrl: 'https://id.nib-cf-test.com/oauth/token',
            clientId: 'test-client-id',
          },
        },
