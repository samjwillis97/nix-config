...
describe('VariableResolver - Phase 7 Plugin Support', () => {
  ...
  describe('T7.4 & T7.5: Plugin Variable Sources', () => {
    it('should resolve plugin variables with plugins.name.variable syntax', async () => {
      ...
      const context = resolver.createContext({}, undefined, undefined, undefined, pluginSources);

      const result = await resolver.resolve(
        'Token: {{plugins.testPlugin.getToken}}, User: {{plugins.testPlugin.getUserId}}',
        context
      );
    ...
    it('should throw error for undefined plugin variables', async () => {
      ...
      await expect(
        resolver.resolve('{{plugins.testPlugin.nonExistentVar}}', context)
      ).rejects.toThrow(VariableResolutionError);

      await expect(
        resolver.resolve('{{plugins.nonExistentPlugin.someVar}}', context)
      ).rejects.toThrow(VariableResolutionError);
    });
    ...
    it('should throw error for plugin variable resolution failures', async () => {
      ...
      await expect(resolver.resolve('{{plugins.errorPlugin.errorVar}}', context)).rejects.toThrow(
        "Plugin variable 'plugins.errorPlugin.errorVar' failed to resolve: Plugin variable error"
      );
    });
    ...
    it('should handle plugin variables in complex objects', async () => {
      ...
      const complexObject = {
        ...
        metadata: ['{{plugins.testPlugin.userId}}', '{{plugins.testPlugin.timestamp}}'],
      };
  ...
  describe('Variable Precedence with Plugins', () => {
    ...
    it('should handle mixed variable sources in same template', async () => {
      ...
      const template =
        'CLI: {{cliVar}}, Profile: {{profile.profileVar}}, API: {{api.apiVar}}, Endpoint: {{endpoint.endpointVar}}, Plugin: {{plugins.authPlugin.getToken}}';
      const result = await resolver.resolve(template, context);

      expect(result).toBe(
        'CLI: cli-value, Profile: profile-value, API: api-value, Endpoint: endpoint-value, Plugin: plugin-token'
      );
    });
  });
  ...
  describe('Plugin Variable Edge Cases', () => {
    ...
    it('should handle empty plugin variable sources', async () => {
      ...
      await expect(resolver.resolve('{{plugins.nonExistent.var}}', context)).rejects.toThrow(
  ...
  describe('Async Variable Resolution', () => {
    it('should handle multiple async variables in same template', async () => {
      ...
      const result = await resolver.resolve(
        '{{plugins.asyncPlugin.var1}} and {{plugins.asyncPlugin.var2}}',
        context
      );
