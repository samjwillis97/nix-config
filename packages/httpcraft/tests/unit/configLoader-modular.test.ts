...
`
      ...
      await fs.writeFile(
        path.join(apisDir, 'service2.yml'),
        `
service2:
  baseUrl: "https://api.service2.com"
  endpoints:
...
`
    ...
    it('should load files in alphabetical order for deterministic merging', async () => {
      ...
      await fs.writeFile(
        path.join(apisDir, 'a-first.yaml'),
        `
testApi:
...
`
      ...
      await expect(configLoader.loadConfig(configFile)).rejects.toThrow('API directory not found');
    });
...
`
      ...
      await expect(configLoader.loadConfig(configFile)).rejects.toThrow('Failed to load API file');
    });
...
`
      ...
      await expect(configLoader.loadConfig(configFile)).rejects.toThrow('baseUrl is required');
    });
...
`
      ...
      // Create chain files
      await fs.writeFile(
        path.join(chainsDir, 'workflow1.yaml'),
        `
workflow1:
...
`
      ...
      await fs.writeFile(
        path.join(chainsDir, 'workflow2.yaml'),
        `
workflow2:
...
`
    ...
    it('should handle both .yaml and .yml files in directory', async () => {
      ...
      await fs.writeFile(
        path.join(chainsDir, 'chain1.yaml'),
        `
chain1:
  steps:
    - id: step1
...
`
      ...
      await fs.writeFile(
        path.join(chainsDir, 'chain2.yml'),
        `
chain2:
  steps:
    - id: step1
...
`
    ...
    it('should load files in alphabetical order for deterministic merging', async () => {
      ...
      await fs.writeFile(
        path.join(chainsDir, 'a-first.yaml'),
        `
testChain:
...
`
      ...
      await fs.writeFile(
        path.join(chainsDir, 'z-last.yaml'),
        `
testChain:
...
`
  ...
  describe('Mixed Import Types', () => {
    it('should handle mixed directory and file imports', async () => {
      ...
      await fs.writeFile(
        path.join(chainsDir, 'dir-chain.yaml'),
        `
dirChain:
...
`
      ...
      await expect(configLoader.loadConfig(configFile)).rejects.toThrow('steps array is required');
    });
...
`
      ...
      await expect(configLoader.loadConfig(configFile)).rejects.toThrow('id is required');
    });
...
describe('ConfigLoader - Modular Profile Imports', () => {
  ...
  describe('Directory Import Support', () => {
    it('should load profiles from directory using "directory:" syntax', async () => {
      ...
      await fs.writeFile(
        path.join(profilesDir, 'development.yaml'),
        `
development:
...
`
      ...
      await fs.writeFile(
        path.join(profilesDir, 'production.yaml'),
        `
production:
...
`
    ...
    it('should merge profiles from multiple files in deterministic order', async () => {
      ...
      await fs.writeFile(
        path.join(profilesDir, 'a-base.yaml'),
        `
base:
...
`
      ...
      await fs.writeFile(
        path.join(profilesDir, 'b-override.yaml'),
        `
base:
...
`
  ...
  describe('Mixed Import Types', () => {
    it('should handle mixed directory and file imports', async () => {
      ...
      await fs.writeFile(
        path.join(profilesDir, 'dev.yaml'),
        `
dev:
  env: "development"
  debug: true
...
`
    ...
    it('should handle later imports overriding earlier ones', async () => {
      ...
      await fs.writeFile(
        path.join(profilesDir, 'base.yaml'),
        `
shared:
...
`
      ...
      await expect(configLoader.loadConfig(configFile)).rejects.toThrow(
        /Invalid profile variable 'invalidKey'/
      );
    });
...
`
      ...
      await expect(configLoader.loadConfig(configFile)).rejects.toThrow(
        /Profile import specification must be a string/
      );
    });
  });
