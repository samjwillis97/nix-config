...
describe('VariableResolver - T10.15 Parameterized Plugin Functions', () => {
  ...
  describe('Function Execution', () => {
    ...
    it('should execute parameterized functions with variable arguments', async () => {
      ...
      const result = await resolver.resolve(
        '{{plugins.auth.getToken("{{username}}", "{{role}}")}}',
        context
      );
    ...
    it('should handle empty argument list', async () => {
      ...
      const result = await resolver.resolve('User: {{plugins.utils.url()}}', context);
    ...
    it('should throw error for undefined parameterized function', async () => {
      ...
      await expect(
        resolver.resolve('{{plugins.test.nonExistentFunc("arg")}}', context)
      ).rejects.toThrow(VariableResolutionError);

      await expect(
        resolver.resolve('{{plugins.nonExistentPlugin.func("arg")}}', context)
      ).rejects.toThrow(VariableResolutionError);
    });
    ...
    it('should throw error when parameterized function throws', async () => {
      ...
      await expect(
        resolver.resolve('{{plugins.error.throwError("test")}}', context)
      ).rejects.toThrow(
        'Parameterized function \'plugins.error.throwError("test")\' failed to execute: Plugin error: test'
      );
    });
  });
  ...
  describe('Backward Compatibility', () => {
    it('should continue to support non-parameterized plugin functions', async () => {
      const nonParameterizedSources: Record<
        string,
        Record<string, import('../../src/types/plugin.js').VariableSource>
      > = {
        legacy: {
    ...
    it('should handle mixed parameterized and non-parameterized functions from same plugin', async () => {
      ...
      const customResult = await resolver.resolve(
        '{{plugins.mixed.getCustomToken("admin")}}',
        context
      );
