...
describe('OAuth2 Plugin', () => {
  ...
  beforeEach(async () => {
    ...
    mockHashInstance.digest.mockImplementation(() => {
      ...
      const hash =
        inputContent.length.toString() + inputContent.substring(0, 10).replace(/[^a-zA-Z0-9]/g, '');
      return `hash-${hash}`;
    });
  ...
  describe('Plugin Setup', () => {
    it('should register hooks and variable sources successfully', async () => {
      ...
      expect(mockContext.registerVariableSource).toHaveBeenCalledWith(
        'accessToken',
        expect.any(Function)
      );
      expect(mockContext.registerVariableSource).toHaveBeenCalledWith(
      ...
      expect(mockContext.registerParameterizedVariableSource).toHaveBeenCalledWith(
        'getTokenWithScope',
        expect.any(Function)
      );
    });
  ...
  describe('Variable Sources', () => {
    ...
    it('should provide accessToken variable source', async () => {
      ...
      const accessTokenSource = mockContext.registerVariableSource.mock.calls.find(
    ...
    it('should provide tokenType variable source', async () => {
      ...
      const tokenTypeSource = mockContext.registerVariableSource.mock.calls.find(
        (call) => call[0] === 'tokenType'
      )[1];
    ...
    it('should provide getTokenWithScope parameterized function', async () => {
      ...
      const getTokenWithScope = mockContext.registerParameterizedVariableSource.mock.calls.find(
        (call) => call[0] === 'getTokenWithScope'
      )[1];
  ...
  describe('Error Handling', () => {
    ...
    it('should throw error for unsupported grant type', async () => {
      ...
      await expect(preRequestHook(mockRequest)).rejects.toThrow(
        'Unsupported OAuth2 grant type: unsupported_grant'
      );
    });
  });
